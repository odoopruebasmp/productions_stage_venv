...
class pos_config(osv.osv):
    ...
    _columns = {
        ...
        'taxes_ids': fields.many2many('account.tax', 'pos_config_tax_rel', 'pos_config_id', 'tax_id', 'Impuestos POS',
                                      domain="[('type', 'in', ['percent','fixed'])]"),
        ...
        'sequence_id': fields.many2one('ir.sequence', 'Order IDs Sequence', help="This sequence is automatically "
                                        "created by Odoo but you can change it to customize the reference numbers of "
                                        "your orders.", copy=False),
        'session_ids': fields.one2many('pos.session', 'config_id', 'Sessions'),
        'group_by': fields.boolean('Group Journal Items', help="Check this if you want to group the Journal Items by "
                                                               "Product while closing a Session"),
        'pricelist_id': fields.many2one('product.pricelist', 'Pricelist', required=True),
        'company_id': fields.many2one('res.company', 'Company', required=True),
        'barcode_product':  fields.char('Product Barcodes', size=64, help='The pattern that identifies product '
                                                                          'barcodes'),
        'barcode_cashier':  fields.char('Cashier Barcodes', size=64, help='The pattern that identifies cashier login '
                                                                          'barcodes'),
        'barcode_customer': fields.char('Customer Barcodes', size=64, help='The pattern that identifies customer\'s '
                                                                           'client card barcodes'),
        'barcode_price':    fields.char('Price Barcodes', size=64, help='The pattern that identifies a product with a '
                                                                        'barcode encoded price'),
        'barcode_weight':   fields.char('Weight Barcodes', size=64, help='The pattern that identifies a product with '
                                                                         'a barcode encoded weight'),
        'barcode_discount': fields.char('Discount Barcodes', size=64, help='The pattern that identifies a product '
                                                                           'with a barcode encoded discount'),
        'negative_amounts': fields.boolean('Permitir cantidades negativas', help='Marque este check si desea permitir '
                                                                                 'movimientos negativos de existencias'),
        'default_partner_id': fields.many2one('res.partner', 'Cliente default'),
        }
...
class pos_session(osv.osv):
    ...
    _columns = {
        ...
        'cash_register_balance_end' : fields.related('cash_register_id', 'balance_end',
                ...
                readonly=True),
        'journal_ids': fields.related('config_id', 'journal_ids', type='many2many', readonly=True,
                                      relation='account.journal', string='Available Payment Methods'),
        'taxes_ids': fields.related('config_id', 'taxes_ids', type='many2many', readonly=True, relation='account.tax',
                                    string='Impuestos'),
    ...
    def login(self, cr, uid, ids, context=None):                        #3. ***Iniciar o Abrir/Reanudar Sesión***
        this_record = self.browse(cr, uid, ids[0], context=context)
        this_record.write({
            'login_number': this_record.login_number+1,
        })

    def wkf_action_open(self, cr, uid, ids, context=None):              #2. ***Validar y Abrir Sesión***BOTÓN ABRIR SESIÓN********
    ...
    def wkf_action_opening_control(self, cr, uid, ids, context=None):      #1. ***NUEVA SESIÓN***
        return self.write(cr, uid, ids, {'state' : 'opening_control'}, context=context)

    def wkf_action_closing_control(self, cr, uid, ids, context=None):    #4. ***CERRAR SESIÓN***FIN DE LA SESIÓN*****
    ...
    def wkf_action_close(self, cr, uid, ids, context=None):             #5. ***Validar y Contabilizar asiento de cierre*****
        # Close CashBox
        for record in self.browse(cr, uid, ids, context=context):
            for st in record.statement_ids:
                # if (abs(st.difference) != 0.0) and st.journal_id.cash_control:
                #     raise osv.except_osv(_('Error!'), _("Debe ajustar la Caja para Validar y Contabilizar el Cierre. "
                #                                         "Existe una Diferencia de (%.2f). ") % (st.difference))
                # if abs(st.difference) > st.journal_id.amount_authorized_diff:
                #     # The pos manager can close statements with maximums.
                #     if not self.pool.get('ir.model.access').check_groups(cr, uid, "point_of_sale.group_pos_manager"):
                #         raise osv.except_osv( _('Error!'),
                #             _("Your ending balance is too different from the theoretical cash closing (%.2f), "
                ...
                if st.journal_id.type not in ['bank', 'cash']:
                    raise osv.except_osv(_('Error!'), _("The type of the journal for your payment method should be "
                                                        "bank or cash "))
    ...
    def _confirm_orders(self, cr, uid, ids, context=None):
        ...
        for session in self.browse(cr, uid, ids, context=context):
            ...
            discount_order_ids = [order.id for order in session.order_ids if order.amount_total < 0]
            for disc in discount_order_ids:
                if disc in order_ids:
                    order_ids.remove(disc)
            if discount_order_ids:
                journal_return = session.config_id.picking_type_id.journal_return_invoice_id
                if not journal_return:
                    picking_type = session.config_id.picking_type_id
                    raise osv.except_osv(("Se encontraron ordenes de devolucion en esta sesion, favor configurar el "
                                          "Diario de Devolucion de Facturacion en la nota de entrega %s") %
                                         picking_type.name)

                for o in discount_order_ids:
                    ref = session.name + ' ' + session.order_ids[session.order_ids.ids.index(o)].name
                    account_move_name = session.order_ids[session.order_ids.ids.index(o)].name
                    move_id = pos_order_obj._create_account_move(
                        cr, uid, session.start_at, ref, journal_return.id,
                        company_id, account_move_name, context=context
                    )
                    pos_order_obj._create_account_move_line(
                        cr, uid, o, session, move_id, context=local_context
                    )
            ...
            for o in order_ids:
                ...
                account_move_name = session.order_ids[session.order_ids.ids.index(o)].name
                move_id = pos_order_obj._create_account_move(
                    cr, uid, session.start_at, ref,
                    session.config_id.journal_id.id, company_id,
                    account_move_name, context=context
                )
                pos_order_obj._create_account_move_line(cr, uid, [o], session, move_id, context=local_context)
                pos_order_obj._create_picking_account_move(cr, uid, [o], context=local_context)

            for order in session.order_ids:
                #if not order.picking_id.account_move_id:
                #    order.picking_id.do_transfer_button()
                if order.state == 'done':
                    continue
...
class PosSession(models.Model):
    ...
    cash_register_balance_end_real = fields2.Float(string="Ending Balance", digits=dp.get_precision('Account'),
                                                   help="Total of closing cash control lines.", compute='_get_detail')
    cash_register_difference = fields2.Float(string='Difference', compute='_get_detail')

    @api.one
    @api.depends('details_ids.subtotal_closing')
    def _get_detail(self):
        for rec in self:
            total = sum([x.subtotal_closing for x in rec.details_ids])
            rec.cash_register_balance_end_real = total
            rec.cash_register_difference = total - rec.cash_register_balance_end
...
class pos_order(osv.osv):
    ...
    def _amount_line_tax(self, cr, uid, line, context=None):
        ...
        taxes_ids = [tax for tax in line.order_id.session_id.taxes_ids]
        ptaxes_ids = [tax for tax in line.product_id.pos_taxes_ids]
        ctaxes_ids = [tax for tax in line.product_id.categ_id.pos_taxes_ids]
        if ptaxes_ids:
            taxes_ids = list(set(line.product_id.pos_taxes_ids) & set(taxes_ids))
        elif ctaxes_ids:
            taxes_ids = list(set(line.product_id.categ_id.pos_taxes_ids) & set(taxes_ids))
        else:
            taxes_ids = []
    ...
    def _process_order(self, cr, uid, order, context=None):
        ...
        if not order['partner_id'] and session.config_id.default_partner_id:
            order['partner_id'] = session.config_id.default_partner_id.id
    ...
    def check_availability(self, cr, uid, orders, context=None):
        # raise self.exception  TODO ojo
        for o in orders:
            cr.execute("SELECT pc.negative_amounts,pc.stock_location_id FROM pos_config pc INNER JOIN pos_session ps "
                       "ON pc.id=ps.config_id WHERE ps.id=%s" % o['data']['pos_session_id'])
            location_id = cr.fetchone()
            if location_id and location_id[0]:
                return 1
            for line in o['data']['lines']:
                cr.execute("SELECT sum(qty) FROM stock_quant WHERE product_id=%s AND location_id=%s" %
                           (line[2]['product_id'], location_id[1]))
                available_qty = cr.fetchone()[0]
                if available_qty < line[2]['qty']:
                    cr.execute("SELECT name_template FROM product_product WHERE id=%s" % line[2]['product_id'])
                    p_name = cr.fetchone()[0]
                    return p_name
        return 1
    ...
    def create_from_ui(self, cr, uid, orders, context=None):
        ...
        # mga modif 2 lines
        # if self.check_availability(cr, uid, orders, context=context) != 1:
            # return False
        order_ids = []
    ...
    _columns = {
        ...
        'lines': fields.one2many('pos.order.line', 'order_id', 'Order Lines', copy=True),
    ...
    def create_picking(self, cr, uid, ids, context=None):
        """
        Picking is not created, this is created in the validating and posting closing entries process
        Only move are created
        """
        partner_obj = self.pool.get('res.partner')
        ...
        for order in self.browse(cr, uid, ids, context=ctx):
            ...
            if order.partner_id:
                ...
                if not picking_type.default_location_dest_id:
                    raise osv.except_osv(_('Error!'), _(
                        'Missing source or destination location for picking type %s. Please configure those fields '
                        'and try again.' % (
                        picking_type.name,)))
                destination_id = picking_type.default_location_dest_id.id
            else:
                destination_id = partner_obj.default_get(cr, uid, ['property_stock_customer'], context=ctx)[
            ...
            if move_list:
                move_obj.action_confirm(cr, uid, move_list, context=ctx)
                move_obj.action_done(cr, uid, move_list, context=ctx)
        return True
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        account_def = journal.default_debit_account_id
        args['account_id'] = (order.partner_id and account_def and account_def.id) or (account_def and account_def.id) or False
    ...
    def action_invoice(self, cr, uid, ids, context=None):
        ...
        for order in self.pool.get('pos.order').browse(cr, uid, ids, context=context):
            ...
            acc = order.statement_ids[0].id if order.statement_ids else False
            inv = {
                'name': order.name,
                'origin': order.name,
                'account_id': acc,
    ...
    def _create_account_move(self, cr, uid, dt, ref, journal_id, company_id,
        name, context=None):
        ...
        return self.pool['account.move'].create(cr, uid, create_dict,
            context=context)
    ...
    def _create_account_move_line(self, cr, uid, ids, session=None, move_id=None, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            i = 0
            while i < len(order.statement_ids):
                journal = order.statement_ids[i].journal_id
                if order.account_move or order.state != 'paid':
                    break

                current_company = order.sale_journal.company_id
                group_tax = {}
                order_account = journal.default_debit_account_id

                if move_id is None:
                    # Create an entry for the sale
                    ses = order.session_id
                    move_id = self._create_account_move(
                        cr, uid, ses.start_at, ses.name + '-' + order.name,
                        order.sale_journal.id, order.company_id.id, None,
                        context=context
                    )
                ...
                def insert_data(data_type, values):
                    ...
                    if data_type == 'product':
                        ...
                        key = ('tax', values['partner_id'], values['tax_code_id'], values['name'], values['debit'] > 0)
                ...
                for line in order.lines:
                    tax_amount = 0
                    taxes = [tax for tax in line.order_id.session_id.taxes_ids]
                    ptaxes = [tax for tax in line.product_id.pos_taxes_ids]
                    ctaxes = [tax for tax in line.product_id.categ_id.pos_taxes_ids]
                    if ptaxes:
                        taxes = list(set(line.product_id.pos_taxes_ids) & set(taxes))
                    elif ctaxes:
                        taxes = list(set(line.product_id.categ_id.pos_taxes_ids) & set(taxes))
                    else:
                        taxes = []
                    computed_taxes = account_tax_obj.compute_all(cr, uid, taxes, line.price_unit *
                                                                 (100.0-line.discount) / 100.0, line.qty)['taxes']
                    ...
                    # Search for the income or return account
                    if line.price_subtotal > 0:
                        account = line.product_id.property_account_income.id or \
                                  line.product_id.categ_id.property_account_income_categ.id
                        if not account:
                            raise osv.except_osv(_('Error!'), _('Please define income account for this product: "%s" '
                                                            '(id:%d).') % (line.product_id.name, line.product_id.id, ))
                    else:
                        account = line.product_id.account_return_id.id or \
                                             line.product_id.categ_id.account_return_id.id
                        if not account:
                            raise osv.except_osv(_('Error!'), _('Por favor defina la cuenta de devolución en el '
                                                                'producto %s o en su categoría') % line.product_id.name)
                    ...
                    insert_data('product', {
                        'name': line.product_id.default_code or line.product_id.name_template,
                        'quantity': line.qty,
                        'product_id': line.product_id.id,
                        'account_id': account,
                    ...
                    for tax in computed_taxes:
                        ...
                        insert_data('tax', {
                            ...
                            'product_id':line.product_id.id,
                            'quantity': line.qty,
                            'account_id': tax['account_collected_id'] or account,
                            'credit': 0.0,
                            'debit': 0.0,
                            'tax_code_id': tax_code_id,
                            'tax_amount': tax_amount,
                ...
                for key, tax_amount in group_tax.items():
                    ...
                    insert_data('tax', {
                        ...
                        'quantity': line.qty,
                        'product_id': line.product_id.id,
                        'account_id': key[account_pos] or account,
                        'credit': ((tax_amount > 0) and tax_amount) or 0.0,
                        'debit': ((tax_amount < 0) and - tax_amount) or 0.0,
                        'tax_code_id': key[tax_code_pos],
                        'tax_amount': abs(tax_amount) * tax.tax_sign if tax_amount >= 0 else abs(tax_amount) *
                                                                                             tax.ref_tax_sign,
            ...
            session_diff = order.session_id.cash_register_difference

            for st in order.statement_ids:
                if st.journal_id not in st_group:
                    st_group[st.journal_id] = st.amount
                else:
                    st_group[st.journal_id] += st.amount

            for st in st_group:
                if st.type == 'cash':
                    if st.amount_authorized_diff <= session_diff:
                        raise Warning("El valor de la diferencia en efectivo supera el valor permitido en la "
                                      "caja asignada para el punto de venta")
                aml_data.append({'name': "PAGOS RECIBIDOS",
                    'account_id': st.default_debit_account_id.id,
                    'credit': ((st_group[st] < 0) and -st_group[st]) or 0.0,
                    'debit': ((st_group[st] > 0) and st_group[st]) or 0.0,
                    'partner_id': order.partner_id and self.pool.get("res.partner")._find_accounting_partner(order.partner_id).id or False,
                    'date': order.date_order[:10],
                    'ref': order.name,
            ...
            if aml_data:
                for aml in aml_data:
                    all_lines.append((0, 0, aml))
            #  TODO diferencias a ajuste al peso
            aml_diff = sum([x[2]['debit'] - x[2]['credit'] for x in all_lines])

            if aml_diff and not -0.01 < aml_diff < 0.01:
                raise Warning("Existe una diferencia entre la venta y los pagos registrados en la orden {o}, "
                              "para ello debe dirigirse a la orden y validar que el valor total de la venta "
                              "corresponda al pagado. \n "
                              "Dando clic en la opcion Más/Facturas obtiene un acceso "
                              "directo a las ordenes relacionadas a esta sesion".format(o=order.name))

            if move_id:  # In case no order was ch340anged
                self.pool.get("account.move").write(cr, uid, [move_id], {'line_id': all_lines}, context=context)
                move = account_move_obj.browse(cr, uid, move_id, context=context)
                if move.journal_id.entry_posted:
                    move.post()

    def _create_picking_account_move(self, cr, uid, orders, context=None):
        """
        :param orders: list ids
        :return: True
        """
        ...
        prec = precision_obj.precision_get(cr, uid, 'Account')
        doc_date = datetime.now()
        ctx = context.copy()
        ctx['bypass'] = True
        for order in self.browse(cr, uid, orders, context=ctx):
            if all(t == 'service' for t in order.lines.mapped('product_id.type')):
                continue
            # Creación y escritura de picking en moves
            addr = order.partner_id and partner_obj.address_get(cr, uid, [order.partner_id.id], ['delivery']) or {}
            picking_type = order.picking_type_id
            picking_date = order.date_order
            picking_id = picking_obj.create(cr, uid, {
                'origin': order.name,
                'partner_id': addr.get('delivery', False),
                'date_done': picking_date,
            ...
            self.write(cr, uid, [order.id], {'picking_id': picking_id}, context=ctx)
            move_list = self.pool.get('stock.move').search(cr, uid, [('origin', '=', order.id)], context=ctx)
            move_obj.write(cr, uid, move_list, {'picking_id': picking_id}, context=ctx)
            picking_obj.action_confirm(cr, uid, [picking_id], context=ctx)
            picking_obj.action_done(cr, uid, [picking_id], context=ctx)

            # Creación asiento contable de Inventario
            pick = picking_obj.browse(cr, uid, picking_id, context=ctx)
            journal = order.picking_type_id.journal_id
            period = period_obj.search(cr, uid, [('state', '=', 'draft'), ('date_start', '<=', picking_date),
                                                 ('date_stop', '>=', picking_date)], limit=1, context=ctx)
            if not period:
                raise osv.except_osv(_('Configuración !'), _("No existe un periodo valido para crear el asiento "
                                        "contable de inventario; por favor validar."))
            if pick.picking_type_id.code == "customer":
                raise osv.except_osv(_('Configuration !'), _(
                    "El sistema no puede transferir un movimiento de inventario que no esta alineado a su tipo de "
                    "operacion, por favor valir el tipo de operacion que quiere realizar y la ubicacion origen y "
            ...
            move_id = account_move_obj.create(cr, uid, move, context=ctx)
            cr.execute("UPDATE stock_picking SET account_move_id = %s WHERE id = %s" % (move_id, picking_id))
            for move in pick.move_lines:
                product_data = move.product_id
                product_qty = move.product_qty
                output_account_id = product_data.property_stock_account_output.id or \
                                    product_data.categ_id.property_stock_account_output_categ.id
                if not output_account_id:
                    raise osv.except_osv(_('Configuración !'), _(
                        "La cuenta de salida de almacen no esta definida en el producto %s ni en su ""categoria" %
                        product_data.name))

                # Contabilidad Analitica por default
                if pick.company_id.analytic_default:
                    account_analytic_id = move.sale_line_id.account_analytic_id.id
                    if not account_analytic_id:
                        cr.execute("SELECT analytic_id FROM account_analytic_default WHERE product_id = %s AND user_id"
                                   " = %s LIMIT 1" % (move.product_id.id, pick.create_uid.id or False))
                        analytic_id = cr.fetchone()
                        if not analytic_id is None:
                            if analytic_id[0]:
                                cr.execute("UPDATE stock_move SET account_analytic_id = %s WHERE id = %s" % (analytic_id[0], move.id))
                        else:
                            error = "El sistema esta configurado para requerir la cuenta analitica en el proceso de "
                            error += "remision, sin embargo no logra encontrar una cuenta para el producto %s y usuario <%s>. "%(move.product_id.default_code, pick.create_uid.name)
                            error += "Esto puede ser porque no tiene valores predeterminados o por que el pedido de "
                            error += "venta fue generado por un usuario que no es comercial." 
                            #raise osv.except_osv(_('Configuración !'), _(
                            #    "El sistema esta configurado para requerir la cuenta analitica en el proceso de "
                            #    "remision, sin embargo no logra encontrar una cuenta para el producto %s y usuario. "
                            #    "%s. Esto puede ser porque no tiene valores predeterminados o por qué el pedido de "
                            #    "venta fue generado por un usuario que no es comercial." % (move.product_id.default_code,
                            #        pick.create_uid.name)) ) 
                            raise osv.except_osv(_('Configuración !'), _(error) )
                else:
                    account_analytic_id = move.account_analytic_id.id or False
                account_id = product_data.cogs_account_id.id or product_data.categ_id.cogs_account_id.id or False
                if not account_id:
                    raise osv.except_osv(_('Configuración !'), _(
                ...
                if move.product_id.valuation == 'real_time':
                    raise osv.except_osv(_('Configuration !'), _(
                        "Cambiar configuracion de evaluacion del producto %s, "
                        "modo tiempo real no soportado" % product_data.name))
                ...
                cr.execute("UPDATE stock_move SET cost = %s, total_cost = %s, costo_promedio = %s WHERE id= %s" %
                           (price, price_total, price, move.id))
                dval, cval = (0, round(price_total, prec)) if order.amount_total > 0 else (round(price_total, prec), 0)
                cr_move_val = {
                    'company_id': order.company_id.id,
                    'journal_id': journal.id,
                    'name': product_data.default_code or product_data.name,
                    'ref': product_data.name[:31],
                    'ref1': product_data.name[:31],
                    'period_id': period[0],
                ...
                avancys_orm.direct_create(cr, uid, 'account_move_line', [cr_move_val, dr_move_val],
                                          company=True)
            account_move_obj.browse(cr, uid, move_id, context=ctx).post()
        return True
...
class PosOrder(models.Model):
    ...
    def get_inv_number(self):
        session = self.env['pos.session'].search([('user_id', '=', self._uid), ('state', '=', 'opened')], limit=1)
        if session:
            seq = session.config_id.sequence_id
            seq_id = seq.id
            # prefix = seq.prefix
            # increment = seq.number_increment
        else:
            sql = "SELECT id,prefix,number_increment from ir_sequence WHERE code = 'pos.order' and active=True"
            self.env.cr.execute(sql)
            res = self.env.cr.fetchall()
            seq_id = int(res[0][0])
            # prefix = res[1]
            # increment = int(res[2][0])
        # irsq = 'ir_sequence_' + str(seq_id)
        # self.env.cr.execute("SELECT last_value FROM {irsq}".format(irsq=irsq))
        # inv_nam = self.env.cr.fetchone()[0]
        # inv_nam = str(prefix) + str(inv_nam + increment)
        #inv_nam = self.pool.get('ir.sequence').get_id(self._cr, self._uid, seq_id, 'id', context=None)
        cr = self.env.cr
        cr.execute(
            """select concat(prefix, number_next::varchar)
            from ir_sequence where id = %s;
            """ % seq_id)
        inv_nam = cr.fetchone()[0]
        return inv_nam

    @api.multi
    def id(self):
        for line in self.lines:
            line._amount_line_all()
...
class pos_order_line(osv.osv):
    ...
    def _amount_line_all(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            taxes_ids = [tax for tax in line.order_id.session_id.taxes_ids if tax.company_id.id ==
                         line.order_id.company_id.id]
            price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            taxes = account_tax_obj.compute_all(cr, uid, taxes_ids, price, line.qty, product=line.product_id,
...
class product_category(osv.osv):
    ...
    _columns = {
        'pos_taxes_ids': fields.many2many('account.tax', 'product_category_pos_tax_rel', 'category_id', 'tax_id',
                                          'Impuestos', help='Especifique los impuestos a mapear con los impuestos '
                                                            'configurados en el TPV'),
        }
...
class product_template(osv.osv):
    ...
    _columns = {
        ...
        'pos_taxes_ids': fields.many2many('account.tax', 'product_template_pos_tax_rel', 'product_id', 'tax_id',
                                          'Impuestos POS', help='Especifique los impuestos a mapear con los impuestos '
                                                                'configurados en el TPV'),
        'pos_taxes_category_ids': fields.related('categ_id', 'pos_taxes_ids', type='many2many',
                                                 relation='product.category', string='Impuestos POS - Categoría'),
        }
...
class res_partner(osv.osv):
    ...
    def create_from_ui(self, cr, uid, partner, context=None):
        ...
        if partner.get('image', False):
            ...
            partner['image'] = img
        if partner.get('vat', False):
            partner['ref'] = partner['vat']
            partner['vat'] = False
