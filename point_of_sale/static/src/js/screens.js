...
function openerp_pos_screens(instance, module){ //module is instance.point_of_sale
    ...
    module.ClientListScreenWidget = module.ScreenWidget.extend({
        ...
        save_client_details: function(partner) {
            ...
            if (!fields.city_id) {
                alert('La ciudad es un campo obligatorio para crear clientes');
                return;
            }
            if (!fields.email) {
                alert('El email es un campo obligatorio para crear clientes');
                return;
            }
            if (!fields.phone) {
                alert('El telefono es un campo obligatorio para crear clientes');
                return;
            }
            if (!fields.ref) {
                alert('El documento es un campo obligatorio para crear clientes');
                return;
            }
            if (!fields.street) {
                alert('La direccion es un campo obligatorio para crear clientes');
                return;
            }
            if (!fields.zip) {
                alert('El tipo de documento es un campo obligatorio para crear clientes');
                return;
            }
        ...
        display_client_details: function(visibility,partner,clickpos){
            ...
            if(visibility === 'show'){
                contents.empty();
                console.log("partner editing", partner);
    ...
    module.ReceiptScreenWidget = module.ScreenWidget.extend({
        ...
        show: function(){
            ...
            //Se comenta para que no se muestre la ventana de imprimir por error en consecutivo
	    ...
	    var self2 = this
            ...
            var instance2 = openerp;
            new instance2.web.Model("pos.order").call("get_inv_number").then(function (result) {

                order.attributes.name = result;//
                    $('.pos-receipt-container', self2.$el).html(QWeb.render('PosTicket',{
                                widget:self2,
                                order:order,
    ...
    module.PaymentScreenWidget = module.ScreenWidget.extend({
        ...
        validate_order: function(options) {
            ...
            // let OrderLines = currentOrder.get('orderLines').models;
            // for (var i = 0; i < OrderLines.length; i++) {
                // console.log("OL", OrderLines[i]);
                // if (OrderLines[i].prodlot_id && OrderLines[i].prodlot_max < OrderLines[i].quantity) {
                    // let msj = "La cantidad vendida supera la disponibilidad del lote " + OrderLines[i].prodlot_id + " total " + OrderLines[i].prodlot_max;
                    // alert(msj);
                    // return;
                // }
                // console.log("QTY_P", this.pos.db.qty_by_product_id[OrderLines[i].product.id]);
                // if (this.pos.db.qty_by_product_id[OrderLines[i].product.id] < OrderLines[i].quantity) {
                    // let msj = "La cantidad vendida supera la disponibilidad del producto " + OrderLines[i].product.name + " total " + this.pos.db.qty_by_product_id[OrderLines[i].product.id];
                    // alert(msj);
                    // return;
                // }
            // }
