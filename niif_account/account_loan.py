...
class account_loan(osv.osv):
    ...
    _columns = {
        'loan_move_line_niif_ids':fields.one2many('loan.move.line', 'loan_niif_id', 'Loan Depriciation Line'),
        'tir_niif':fields.float(string='TIR Contable', digits_compute= dp.get_precision('Exchange Precision')),
        }
    
    def compute_loan_board_niif(self, cr, uid, ids, context=None):
        ...
        for loan in self.browse(cr, uid, ids, context=context):
            #if loan.tir <= 0.0:
                #loan.compute_loan_board_niif(cr, uid, ids, context=context)
            cuotas_pagadas = 0
            cuotas_gracia_pagadas = 0
            desembolso = False
            cuota_extra = 0
            if loan.loan_move_line_niif_ids:
                for cuota in loan.loan_move_line_niif_ids:
            ...
            for period in range(cuotas_pagadas, loan.periods_to_pay):
                ...
                if loan.dec_type == 'cuoto_fija':
                    capital_payment = ((tasa_interes*((1+tir)**(remain_period))/(((1+tir)**(remain_period))-1))*(balance_debt))-((initial_value-final_payment)*tir)
                else:
...
class loan_move_line(osv.osv):
    ...
    _columns = {
                'loan_niif_id':fields.many2one('account.loan', 'Loan', ondelete='cascade'),
            }
    ...
    #def create_move(self, cr, uid, ids, context=None):
        ...
        #for line in self.browse(cr, uid, ids, context=context):
            ...
            #self.write(cr, uid, line.id, {'move_name': new_move_name, 'move_id': move_id, 'state':'confirm'}, context=context)
...
#class account_loan_distribution(osv.osv):
    #_inherit = "account.loan.distribution"
