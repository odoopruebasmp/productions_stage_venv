...
class account_financial_reports_accounting_auxiliar(models.Model):
    _name = "fpa.auxiliar"
...
class account_financial_reports_accounting_auxiliar_line(models.Model):
...
class wizard_account_financial_reports_accounting_auxiliar(models.TransientModel):
    ...
    period_balance_ids = fields.Many2many('account.period', string='Periodos', help='Período asociado a los '
                                            'movimientos contables a consultar.')
    journal_ids = fields.Many2many('account.journal', string='Diarios', help='Diario asociado a los movimientos '
                                                                             'contables a consultar.')
    account_ids = fields.Many2many('account.account', string='Cuentas', domain=[('type', '!=', 'view')], help='Cuentas '
                                    'contables asociadas a los movimientos a consultar.')
    analytic_ids = fields.Many2many('account.analytic.account', string='Cuentas analiticas', domain=[('type', '!=',
                                                                                                      'view')])
    partner_ids = fields.Many2many('res.partner', string='Terceros', help='Tercero asociado a los movimientos '
                                    'contables a consultar.')
    date_from = fields.Date(string="Fecha Inicial", required=True, help='Fecha inicial de consulta, el informe '
                                'retornará los movimientos contables desde esta fecha.')
    ...
    estado = fields.Selection([('borrador', 'Borrador'), ('validados', 'Validados'), ('todos', 'Todos')],
                              default='todos', string='Estados', required=True, help='Estado en el cual se desean '
                                'consultar los movimientos contables.')
    ...
    def generar(self):
        ...
        self.env.cr.execute(''' select count(*) from fpa_auxiliar_line ''')
        ...
        if count > 1000000:
            self.env.cr.execute(''' TRUNCATE fpa_auxiliar_line ''')
        else:
            self.env.cr.execute(
                ''' DELETE FROM fpa_auxiliar_line WHERE financial_id =%s AND company_id = %s and user_id = %s''' % (
        ...
        movimientos = ''' INSERT INTO fpa_auxiliar_line (bold,user_id,company_id,account_id,cuenta,account_analytic_id,fecha,asiento,linea,partner_id,debit,credit,amount_final,encabezado_id,financial_id, nivel)
                            SELECT False as bold, %s as user_id, %s as company_id, %s::integer as account_id, aa.code,
                                ...
                                aml.partner_id, round(aml.debit,2) as debit, 
                                round(aml.credit,2) as credit, aml.debit-aml.credit as amount_final,
                                %s::integer as encabezado_id, %s as financial_id, 99 as nivel
        ...
        saldo_inicial = ''' INSERT INTO fpa_auxiliar_line (bold,user_id,company_id,account_id,cuenta,amount_inicial,debit, 
        ...
        for structure in self.chart_account_id.structure_id.sorted(key=lambda r: r.sequence, reverse=True):
            if str(structure.sequence) in niveles:
                # nivel
                nivel = '''INSERT INTO fpa_auxiliar_line (bold, user_id,company_id, account_id, cuenta, partner_id, amount_inicial, debit, credit, amount_final,encabezado_id,nivel,financial_id)
        ...
        if '0' in niveles:
            ...
            chart_account = '''INSERT INTO fpa_auxiliar_line (bold,user_id,company_id, account_id, cuenta, partner_id, amount_inicial,
                            ...
                            sum(al.amount_final) as amount_final, %s, 0, %s from fpa_auxiliar_line al where al.nivel=97
        ...
        if '97' not in niveles:
            ...
            self.env.cr.execute(''' DELETE FROM fpa_auxiliar_line WHERE nivel=97 ''')
        if '99' not in niveles:
            # eliminar lineas de movimientos
            self.env.cr.execute(''' DELETE FROM fpa_auxiliar_line WHERE nivel=99 ''')
        # eliminar lineas sin saldo inicial, debito y credito
        self.env.cr.execute(
            ''' DELETE FROM fpa_auxiliar_line WHERE amount_inicial=0 and debit=0 and credit=0 and amount_final=0''')

        # aplicar conversion de miles
        if int(financial_reports.unidades) > 1:
            self.env.cr.execute(
                ''' UPDATE fpa_auxiliar_line SET amount_inicial=amount_inicial/{unidades}, debit=debit/{unidades}, 
