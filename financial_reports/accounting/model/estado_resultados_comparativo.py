...
_logger = logging.getLogger('ESTADO DE RESULTADOS (Comparativo)')

class account_financial_reports_accounting_pyg_c(models.Model):
    _name = "fpa.pyg.c"
...
class account_financial_reports_accounting_pyg_c_line(models.Model):
    ...
    encabezado_id = fields.Many2one('fpa.pyg.c', string='Encabezado', ondelete='cascade')
    financial_id = fields.Many2one('fpa.financial.reports', string='Reporte')
    concepts_id = fields.Many2one('fpa.financial.reports.concepts', string='Conceptos', ondelete='cascade')

    @api.model
    def fields_view_get(self, view_id=None, view_type=False, toolbar=False,submenu=False):
        result =super(account_financial_reports_accounting_pyg_c_line,self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        # for reg in self:
        #     _logger.info('result')
        #     _logger.info(result)
        #     if reg.env.context:
        ...
        #         encabezado_id = reg.env['fpa.pyg.c'].search([('financial_id','=',reg.env.context.get('active_id',False)),
...
class wizard_account_financial_reports_accounting_pyg_c(models.TransientModel):
    ...
    def generar(self):
        ...
        self.env.cr.execute(''' select count(*) from fpa_pyg_c_line ''')
        ...
        if count > 1000000:
            self.env.cr.execute(''' TRUNCATE fpa_pyg_c_line ''')
            self.env.cr.execute(''' TRUNCATE fpa_pyg_c ''')
        else:
            self.env.cr.execute(''' DELETE FROM fpa_pyg_c_line WHERE financial_id=%s AND company_id = %s and user_id = %s''' % (financial_reports.id, self.company_id.id, self.env.user.id))
            self.env.cr.execute(''' DELETE FROM fpa_pyg_c WHERE financial_id=%s AND company_id = %s and user_id = %s''' % (financial_reports.id, self.company_id.id, self.env.user.id))
        where = ''
        cuenta = []
        #Cuentas
        ...
        encabezado_id = self.env['fpa.pyg.c'].create(header)
        _logger.info(header)
        encabezado_id = encabezado_id.id

        # Agrega encabezado con parametros indicados por el usuario
        #sql = " INSERT INTO fpa_pyg_c(date, date_from, date_to, estado, company_id, user_id,chart_account_id, financial_id,amount_comparative_label) VALUES ('%s','%s','%s','%s',%s,%s,%s,%s) RETURNING ID " %(datetime.now(), self.date_from, self.date_to, self.estado, self.company_id.id, self.env.user.id, self.chart_account_id.id,financial_reports.id)
        ...
        #Movimientos
        movimientos = " INSERT INTO fpa_pyg_c_line (nivel,sequence,bold,user_id,company_id,account_id,cuenta,concepts_id,debit, "\
                            ...
                            " GROUP BY ffrc.sequence, account_id, aa.code, ffrc.id " % (self.env.user.id, self.company_id.id, encabezado_id, 
                                financial_reports.id, account, self.env.user.company_id.id, self.date_from, self.date_to, where, account, 
        ...
        self.env.cr.execute(''' UPDATE fpa_pyg_c_line SET 
                                        ...
                                        SELECT 
                                            ...
                                            FROM 
                                                (
                                                SELECT  datos.account_id, amount as amount_comparative--(CASE WHEN datos.num::int = 1 THEN datos.amount ELSE 0 END) as amount_comparative
                                                        FROM (
                                                            SELECT period_range.num, aml.account_id, SUM(debit-credit) as amount
                                                                FROM account_move_line aml 
                                                                    INNER JOIN (
                                                                    ...
                                                                    WHERE aml.account_id IN (SELECT account_id FROM fpa_pyg_c_line WHERE financial_id = %s)
                                                            GROUP BY period_range.num, aml.account_id) as datos
                                                ) as result GROUP BY account_id) as resultado
                                    WHERE fpa_pyg_c_line.financial_id = %s AND fpa_pyg_c_line.user_id = %s AND fpa_pyg_c_line.company_id =%s
                                    AND fpa_pyg_c_line.account_id = resultado.account_id '''%(self.period_range.id,financial_reports.id,financial_reports.id,self.env.user.id,self.env.user.company_id.id))
        self.env.cr.execute(''' UPDATE fpa_pyg_c_line SET variation = amount_final-amount_comparative WHERE user_id=%s AND financial_id =%s 
                                    AND company_id=%s '''%(self.env.user.id,financial_reports.id,self.env.user.company_id.id))
        if '100' in niveles:
            #Agregar totales por concepto incluyendo los comparativos
            sql = "INSERT INTO fpa_pyg_c_line (nivel,bold,user_id,company_id,account_id,cuenta,debit, "\
                " credit,amount_final,amount_comparative,variation,encabezado_id,resume,concepts_id,financial_id) "\
                " SELECT 100,True, %s, %s, null, null, SUM(debit),SUM(credit), SUM(amount_final), SUM(amount_comparative),SUM(variation),%s, False, ffrc.id, %s "\
                " FROM fpa_financial_reports_concepts ffrc "\
                " LEFT JOIN fpa_pyg_c_line fpl ON ffrc.id = fpl.concepts_id "\
        ...
        sql = " UPDATE fpa_pyg_c_line SET porc_variation=(CASE WHEN amount_comparative = 0 THEN (CASE WHEN variation<0 THEN -1 ELSE 1 END) ELSE variation/amount_comparative END)*100 WHERE user_id=%s AND financial_id=%s AND company_id=%s "%(self.env.user.id,financial_reports.id,self.env.user.company_id.id)
        _logger.info(sql)
        self.env.cr.execute(sql)

        if '99' not in niveles:
            self.env.cr.execute(" DELETE FROM fpa_pyg_c_line WHERE nivel=99 AND user_id=%s AND financial_id =%s AND company_id=%s"%(self.env.user.id,financial_reports.id,self.env.user.company_id.id))

        #Eliminar lineas sin saldo inicial, débitos y créditos
        self.env.cr.execute(" DELETE FROM fpa_pyg_c_line WHERE amount_comparative=0 AND debit=0 AND credit=0 AND amount_final=0 AND user_id=%s AND financial_id =%s AND company_id=%s"%(self.env.user.id,financial_reports.id,self.env.user.company_id.id))
        
        #cambia el signo
        if financial_reports.sign and 1==2:
            sql = " UPDATE fpa_pyg_c_line SET amount_final = amount_final * -1, amount_comparative = amount_comparative*-1, variation=variation*-1 WHERE user_id=%s AND financial_id =%s AND company_id=%s"%(self.env.user.id,financial_reports.id,self.env.user.company_id.id)
            self.env.cr.execute(sql)

        if financial_reports.unidades > 1:
            self.env.cr.execute(
                ''' UPDATE fpa_pyg_c_line SET debit=debit/{unidades}, 
